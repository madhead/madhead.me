...
It means even if I manage to get `tar` installed, this solution would not work in the future for other pods unless we add `tar` to the image.
I knew for sure that my team would not be happy with adding extra stuff to the image `tar` just for the sake of copying a file.
